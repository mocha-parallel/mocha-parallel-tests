...
class AbstractReporter extends MochaBaseReporter {
    ...
    _setReporterOnce() {
        ...
        reporterInstance = new UserReporter(customRunner, this._options);
    }

    _storeEventData(evtType, arg) {
        debugLog(`${evtType} event fired (${this._relativeFilePath})`);
    ...
    _listenToRunnerEvents() {
        const storeEventData = this._storeEventData.bind(this);

        this._runner.on('suite', suite => storeEventData('suite', suite));
        this._runner.on('suite end', suite => storeEventData('suiteEnd', suite));
        this._runner.on('test', test => storeEventData('test', test));
        this._runner.on('pending', test => storeEventData('pending', test));
        this._runner.on('pass', test => storeEventData('pass', test));
        this._runner.on('test end', test => storeEventData('testEnd', test));
        
        this._runner.on('fail', (...args) => {
            storeEventData('fail', args);
            failsOccured++;
        });

        this._runner.on('failRetry', () => {
            failsOccured--;
        });
