...
const scheduleMochaRun = test => {
     ...
     */
    mocha.loadFiles();
    debugLog(`Suites length is ${mocha.suite.suites.length} for ${test.file}`);

    console.log('patching desribe from parallel-tests')
    var orig = global.describe;

    // idea here is to patch global.describe so it could leave only 1 suite for each loop
    ...
    //     value: function () {
    //         console.log('run patched describe!');
    //         return orig.apply(this, arguments);
    //     },
    //     writable : false,
    //     enumerable : true,
    ...
    for (let i = 0; i < mocha.suite.suites.length; i++) {
        // suites are being populated by calling describe
        // desctibe itself is set via global.describe in mocha itself
        // the only way we can re-run describes is to clear cache assicoated with the test file
        delete require.cache[path.resolve(test.file)];
