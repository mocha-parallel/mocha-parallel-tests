...
const scheduleMochaRun = test => {
    ...
    const onEnd = suiteIndex => {
        return () => {
            debugLog(`Test processed: ${path.relative(__filename, test.file)} (suite ${suiteIndex})`);

            const runnerId = getRunnerId(test.file, suiteIndex);
    ...
    for (let i = 0; i < Math.max(mocha.suite.suites.length, 1); i++) {
        /**
         * Suites are being populated by calling describe
         * Desctibe itself is set via global.describe in mocha itself
         * The only way we can re-run describes is to clear cache
         * assicoated with the test file
         */
