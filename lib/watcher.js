    ...
    const onFail = suiteIndex => {
        ...
        return testArg => {
            ...
            if (testFilesFailes.get(runnerId) <= retryCount) {
                const relativeFilePath = path.relative(__filename, file);
                originalWrites.stderr(`[${relativeFilePath}]:[${suiteIndex}] try #${testFilesFailes.get(runnerId)} failed: ${testArg.err.message}\n`);
