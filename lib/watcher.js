...
const scheduleMochaRun = ({file, options, throttledCalls}, cacheWatcher) => {
    ...
    const onEnd = suiteIndex => {
        return () => {
            ...
            if (runners.size || pendingTests.length) {
                runTestsRecursive({options, throttledCalls, cacheWatcher});
            }
        };
    };
...
const runTestsRecursive = ({options, throttledCalls, cacheWatcher}) => {
